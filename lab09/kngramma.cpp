...
void tGramma::loadFromFile(const char* filename)
{
    ...
    if (!inputF.is_open()) {
        errmes = "GRAMMA: can't open file " + string(filename);
        return;
    }
    ...
    str[0] = '\0';
    while ((!*str || ((p = strtok(str, delimiters)) == NULL))
        && !inputF.eof()) {
    ...
    while (!inputF.eof()) {
        ...
        if (!broken) {
            ...
            if (e.empty()) {
                ...
                errmes = "GRAMMA: incorrect symbol " + string(p);
                clear();
                return;
            }
    ...
    for (unsigned char i = 1; i < Abc.size(); ++i) {
        string s = decode(i);
        Iabc[s] = i;
        size_t ln = s.length();
        if (ln > Smbwidth)
    ...
    markerCount = 0;
    tSymb left = static_cast<unsigned char>(Start);
    bool active;
    ...
    while (!inputF.eof()) {
        ...
        if (p == NULL || !isPrefix(p, "->")) {
            ...
            errmes = "GRAMMA:expected -> after left part " + decode(left);
            clear();
            return;
        }
        ...
        while (active) {
            ...
            if (p == NULL) {
                ...
                if (isPrefix(p, MARKER.c_str())) {
                    ...
                    currentAlternative.hndl = static_cast<unsigned short>(z);
                    p = strtok(NULL, delimiters);
                    if (p != NULL && strcmp(p, "|")) {
