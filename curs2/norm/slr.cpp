    ...
    tLRitems& closure, tGramma::tSymb a)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        insert_item(closure, tLRI(a, static_cast<unsigned char>(i)).first_point(gr));
    }
}
...
void SLRbuild(tLR& lr)
{
    ...
    tGramma::tSymb smb = static_cast<unsigned char>(gr.getStart());
    for (unsigned char i = 0; i < gr.altCount(smb); ++i) {
        item = tLRI(smb, i).first_point(gr);
        insert_item(kernel, item);
    }
    kernels.push_back(kernel);

    for (short from = 0; from < kernels.size(); ++from) {
        closure.clear();
        size_t to;

        for (unsigned char i = 0; i < kernels[static_cast<size_t>(from)].size(); ++i) {
            tLRI& ritem = kernels[static_cast<size_t>(from)][i];
            if (ritem.smb)
                closure.push_back(ritem);
            else
                lr.add(from, follow, ritem.left, ritem.ialt);
        }
        ...
        if (!closure.empty()) {
            ...
            for (; i < closure.size(); ++i) {
                ...
                lr.add(from, smb, static_cast<short>(to));
                smb = item.smb;
                item.next_point(gr);
                kernel.clear();
                kernel.push_back(item);
